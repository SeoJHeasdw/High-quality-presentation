...
> **진행 기록 (2026-09-23 저녁):** 세션 1~3을 한 번에 진행해 43장·84단계가 됐다. 5절 사실 확인은 F1~F16 모두 원문으로 확인했다
> (F2 게시물 날짜 2026-07-02 UTC, F5 83.3%·11.1%는 v1 31쪽, F15는 Kevin Weil의 Lenny's Podcast 2025-04-10 원문, F14는 무대에서 빼는 근거로만).
> 6절 자산(A1 영수증 두 장, A2 RICE 29.16초 전체, A3 절차적 실루엣)과 7절 번호 재배치를 끝냈다.
> 계획과 다르게 한 점: 18번은 `OneUser` 묶음에 19번까지 넣었다(phase 5~9). 29번은 opening world를 새 인스턴스로 부르는 대신 같은 입자 문법의
> `worst/world.ts`를 따로 두었다(2~3번의 승인된 코드를 건드리지 않기 위해). 42번의 세 노트(01 연결·02 품질·03 생활)는 대본으로 옮겼다.
> 남은 일과 사용자 확인 사항은 [NEXT-STEPS.md](NEXT-STEPS.md)에 있다.
...
- 강의자료 쪽(`src/production/chapters/ch01-open.ts` "32세", `script/course/ch06.md` "32세의 목표")은 별도 강의라 **사용자에게 물은 뒤** 고친다. → 2026-09-23 사용자 확인 후 31세로 고쳤다(`강의안_통합.md` 포함).
