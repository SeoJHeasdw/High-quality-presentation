...
Technology Expert Lab 팀과 나누는 30년 생존 실험. 43장, 84개 발표 단계로 구성한 16:9 브라우저 발표다.
...
4·20·32·36·37·43번에는 React Bits 기반 포인터·클릭 연출이 있다. 1번은 원래 표지로 유지한다.
20번 이미지는 클릭하면 크게 열리고 Escape로 닫힌다. 확대 중에도 방향키로 발표를 이어갈 수 있다.
...
"결국 영상 만드는 툴 아니냐"는 반응에는 구조(18번 공장 안), 동기(14번 과도기), 확장(42번), 속도(29번 가장 구린 모델)로 답한다.

| 장표 | 이야기 |
| --- | --- |
| 1~4 | 고양이 사진을 알아보던 AI, 일을 맡기는 현재, 예측하기 어려운 미래(고양이는 29번의 복선) |
...
| 13~14 | 31세의 30년 눈금, "일이 선택이 된다"는 예측과 그 사이의 과도기, 판단은 제가 쥔다 |
| 15~17 | 실제 강의 24초의 제작 기록, 사용자는 저 한 명, Agent OS와 Factory |
| 18~19 | Factory 안: 일마다 맞춘 로컬 모델과 판단에만 부르는 프론티어 모델, 디지털 월세, "결국 영상 만드는 거 아닌가요?"에 대한 세 가지 베팅 |
| 20~25 | 결과물 갤러리와 Blender로 렌더링한 Factory 영상(화면 통과, 매치 컷, 셔터 전환, 원형 전환) |
| 26~28 | 저장된 실제 강의 영상, 프리비즈, 음악 후보 재생 |
| 29 | "AI로 할 바엔 사람이 낫지 않나요?" → 지금 쓰는 모델이 앞으로 쓸 모델 중 가장 구린 모델(고양이 회수) |
| 30 | 이 발표의 제작 과정 |
| 31~38 | 제가 녹음한 적 없는 제 목소리, 수고가 줄어들 때, 새벽의 사칭 전화·관계 조작·동의 없는 합성, 확인하는 방법 |
| 39 | 신에게 목줄이 걸릴까(발표자의 질문) |
| 40~43 | 같은 집의 새벽과 아침: 바뀌는 모델과 남는 기록, 잘 안됐던 과정의 공개, 계속 늘어나는 방, 만들기·맡기기 질문 |

## 서비스 시연 방식

본편은 **26~28번에 담은 실제 결과물을 장표 진입 시 자동 재생**한다. 26번과 28번은 화면이 나타난 뒤 1초 쉬고 시작한다. `P`는 재생/일시정지,
`A`는 소리 켜기/끄기다. 26번 강의와 28번 음악은 소리가 켜진 상태로 시작하고 27번 프리비즈는 무음 원본이다.
...
21번부터 →로 진행하면 각 장면의 영상이 한 번 재생되고 마지막 화면에서 멈춘다.
...
23번에 쓰는 청록색 새는 이 발표를 위해 Local Assets Engine으로 새로 생성한 2D 이미지다.
이미지 원본·프롬프트·모델·시드는 `public/engines/factory-assets/hero.json`에 있다. 기존 출력이나 엔진 코드는 변경하지 않았다.

20번의 미리보기도 실제 강의 캡처, 새 이미지가 등장하는 장면, 음악 파형 장면으로 맞췄다.
31~38번은 어두운 방과 붉은색(덱의 색 규칙에서 빨강 = 사실이 아님)으로 분위기를 바꾼다(`src/keynote/abuse/`).
34번 두 번째 단계에서 같은 말을 LoRA 강도 0·0.5·1.0으로 만든 목소리가 1초 간격으로 겹치고, 합성 여성 목소리가 마지막 말을 한다. 6.6초짜리 가상 음성 연출이며 35번의 통화 장면은 그대로 유지한다. P는 재생·일시정지, A는 음소거다. 제작 기록은 `public/abuse/audio/threshold-voices.json`에 있다.
31번은 장표 진입 뒤 약 1.35초 쉬고 26번에서 들은 실제 강의의 한 문장을 자동으로 다시 들려준 뒤, "저는 이 문장을 녹음한 적이 없습니다"로 붉게 바꾸고,
제 목소리 어댑터의 실제 학습 기록(12.3분·35.3초·약 67MB)과 공개 모델의 "3초" 설명을 놓는다. P는 이 문장 재생, A는 소리다.
32번은 붉게 물든 픽셀 얼굴, 33번은 지금의 제작 과정, 34번은 네 개의 벽(생성·확인·수정·연결)이 무너지며 검은 군중이 다가오는 도식이다.
35~36번은 Blender로 렌더한 한밤중 협탁 위 휴대전화 한 대(`public/abuse/night-phone.jpg`)에서 이어진다(묶음 night-phone).
...
35번 수신 시 짧은 진동음(WebAudio)이 두 번 나고, `→` 또는 휴대전화의 응답 버튼을 누르면 통화 단계로 넘어가 가상 합성 음성이 한 번 재생된다. 36번 음성 메시지도 장표 진입 시 한 번 재생된다. A는 음소거, P는 재생/정지다.
두 음성은 Qwen3-TTS와 발표자 본인의 LoRA 어댑터(0.60)로 만들었으며 실제 다른 사람의 목소리를 흉내 내지 않는다. 제작 기록은 `public/abuse/audio/manifest.json`에 있다.
37번은 모자이크한 사진 한 장이 두 배씩 늘어 화면을 채우고, 원본을 지워도 사본이 남는다.
38번은 같은 휴대전화에서 끊고 저장된 번호로 다시 거는 확인 방법(FTC 권고)으로 끝난다. 모든 통화·메시지·사진은 가상 상황이다.
배경 이미지의 생성 기록은 `../docs/abuse-art.md`에 있다.
39번은 붉은 방이 아니라 완전한 검정 위의 파랑과 금색이다(`src/keynote/leash/`). 사람을 움직여 온 낱말(약속·설득·명령·prompt…)이 흘러와
사람의 형태가 되고(39-0), 금색 고리가 목을 감으려 좁혀지다가 끝내 닫히지 않고 벌어진다(39-1). 실루엣은 외부 이미지 없이 캔버스에 직접 그려 입자로 샘플링한다.
40~43번은 한 묶음(finale)이다(`src/keynote/finale/`). 12번의 집을 시간대만 바꿔 다시 렌더한 푸른 새벽(40~42, 흐리게)과 해 뜨는 아침(43)이 뒤에 깔리고,
43번에서 배경이 선명해지며 아침으로 밝아진다. 40번은 위로 지나가는 모델(지금 Qwen3-TTS, 이후는 가정)과 아래에 쌓이는 작업 방식·판단의 기록·결과물의 세 겹,
41번은 local-tts-engine 결정 기록에서 뽑은 실제로 잘 안됐던 과정 여섯 줄과 동의 확인 약속이다.
42번은 새벽 하늘 위에 투명한 3D 캔버스를 얹은 설계도다(`finale/blueprint.ts`, 세 단계). 12번의 청사진 격자에서 방이 솟는다: 지어진 방 TTS·ASSETS·MUSIC(개발 중)과
...
43번은 "만들기"와 "맡기기" 두 낱말에 초점 틀이 옮겨 다니는 질문이다. 초점이 가는 쪽에 따라 아침빛의 색이 바뀐다.
...
13·14번은 한 묶음(person-ruler)이다. 13번은 12번이 멈춘 마지막 이미지(불 켜진 집)에서 이어지고, 아래에 2026~2056년(31세~61세)의 30년 눈금을 긋는다. 2031년 눈금은 4번의 질문을 다시 가리킨다.
14번은 같은 집과 눈금을 다시 그리지 않고 글과 눈금 위의 표시만 바꾼다: 머스크의 원문 한 줄과 "2035~2045?" 구간(14-0) → 지금부터 그 구간까지의 금색 과도기와
"다 할 수 있다 · 대체될 수 있다"(14-1) → MIT 미디어랩 연구(83%, 동료심사 전)와 "그래서 판단은 제가 쥡니다"(14-2). 인물 사진이나 게시물을 흉내 낸 카드는 쓰지 않는다.
15번은 실제 강의 24초의 제작 기록이다. 대본 문장·제 목소리 파형·단어 단위 자막·영상 프레임을 같은 시간축에 놓고, 오른쪽 위 영상의 재생 위치를 재생 헤드가 따라간다.
값은 local-tts-engine의 타임라인에서 뽑은 `src/keynote/data/tts-alignment.json`을 쓴다. M에서는 2.4초('AI가') 지점에 멈춘 화면을 보여준다.
16~19번은 three.js 공간 하나를 네 장 동안 유지한다(`src/keynote/personal/world.ts`, 묶음 one-user, phase 0~9). 많은 사용자의 점이 가라앉고 금색 점(저) 하나가 남는다(16-0) →
조건의 고리가 느슨해지고 요구사항 표가 "사용자가 저 한 명일 때"로 옮겨간다. 원본과 작업 기록만 잠겨 있다(16-1) → 카메라가 물러나며 저 → Agent OS(구상, 점선) →
Factory(개발 중) → 강의 영상 → 저의 고리가 차례로 켜지고, 마지막에 "이 페이지 발음만 다시" 요청이 TTS로 되돌아간다(17-0~2) →
카메라가 Factory 안으로 들어가고 엔진이 호를 따라 스테이션 다섯 개(목소리, 받아쓰기 검수, 이미지·3D, 음악, RICE)로 늘어선다. 요청이 오면 맞는 스테이션 하나만 켜지고,
위에 따로 떨어진 문(프론티어 모델)에는 어려운 판단만 올라간다(18-0) → 문 앞에 실제 영수증 두 장(`public/rent/`, 사이드바를 잘라낸 부분)이 내려온다(18-1) →
모든 스테이션이 각자 자기 일을 하고 Compound AI System이라는 이름을 붙인다(18-2) → 공간 전체가 가라앉고 "결국 영상 만드는 거 아닌가요?"(19-0) →
세 가지 베팅: 결과물이 빛나고, 저에게서 금색 고리가 퍼지고, 목소리 스테이션의 모델이 새것으로 바뀐다(19-1, 발표자의 판단).
20번은 21번 Blender 갤러리와 같은 배치(가운데 01 VOICE, 왼쪽 02 IMAGE, 오른쪽 03 MUSIC)의 전시 공간이다. 카드의 기울기·확대 보기는 그대로 쓴다.
29번은 2~3번의 입자 문법을 다시 부른다(`src/keynote/worst/`). 청중의 속마음을 먼저 인정하고(29-0), 화소 부조 고양이가 금색 "고양이"가 된 뒤(한때)
입자가 강의 화면 테두리로 모이고(지금) 일부가 오른쪽 어둠으로 흘러가며 파란 눈금 "1년 뒤?" "2년 뒤?"를 지난다(29-1). 29-2는 근거 두 줄과 "모델이 좋아질수록 같이 좋아지는 공장에 겁니다".
이 공간들은 모두 앞으로 넘길 때만 움직임을 재생하고, ←와 번호 이동은 그 단계의 마지막 상태를 보여준다. M 또는 모션 줄이기 설정에서도 마지막 상태를 표시한다.
WebGL을 쓸 수 없으면 같은 내용을 평면으로 보여준다. 공통 렌더 틀은 `src/keynote/stage3d/runtime.ts`에 있다(42번은 배경 없이 그리는 transparent 옵션).
...
14·18·29·39·42번에 새로 쓴 인용과 수치(머스크 원문, MIT 83%, 영수증 두 장, Compound AI System, 케빈 웨일, a16z 10배, Gemini Robotics, RICE·Personal CIO)의
...
- 13·14번: `src/keynote/personal/PersonalIntro.tsx`(집·30년 눈금·과도기), `personal.css`
- 15번: `src/keynote/personal/ProductionTimeline.tsx`, 제작 기록 `src/keynote/data/tts-alignment.json`, 프레임 띠 `public/demos/tts-strip.jpg`
- 16~19번 공간: `src/keynote/personal/OneUser.tsx`(요구사항 표·제목·라벨·베팅), `personal/world.ts`(고리·Factory 안의 스테이션·프론티어 문·영수증·모델 교체), 영수증 `public/rent/`
- 20번 전시 공간: `src/keynote/PersonalSlides.tsx`, `personal/personal.css`, `EnginePreview.tsx`
- 29번 공간: `src/keynote/worst/WorstModel.tsx`(글·눈금 라벨·근거), `worst/world.ts`(부조·입자·미래로 흐르는 빛), `worst/worst.css`
- 39번 공간: `src/keynote/leash/Leash.tsx`, `leash/world.ts`(낱말 → 몸, 닫히지 않는 고리), `leash/leash.css`
...
- 마무리 구간 40~43번: `src/keynote/finale/Finale.tsx`(네 장의 글·기록), `Blueprint3D.tsx`·`blueprint.ts`(42번 3D 설계도와 RICE 영상), `finale.css`, 43번의 선택지 `src/keynote/ClosingChoices.tsx`
...
발표자 동기화, GPU 비활성 대체 화면, 미디어의 실제 로딩과 재생을 확인한다. 묶음 검사는 13·14번 눈금, 16~19번 공간(phase 0~9),
40~43번 새벽 하늘과 42번 3D 설계도를 포함한다. 두 검사 모두 장표를 번호가 아니라 id로 찾으므로 장표를 끼워도 같은 장면을 가리킨다.
