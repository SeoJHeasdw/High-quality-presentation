...
본편은 **23~25번에 담은 실제 결과물을 장표 진입 시 자동 재생**한다. 23번과 25번은 화면이 나타난 뒤 1초 쉬고 시작한다. `P`는 재생/일시정지,
`A`는 소리 켜기/끄기다. 23번 강의와 25번 음악은 소리가 켜진 상태로 시작하고 24번 프리비즈는 무음 원본이다.
...
30번 두 번째 단계에서 같은 말을 LoRA 강도 0·0.5·1.0으로 만든 목소리가 1초 간격으로 겹치고, 합성 여성 목소리가 마지막 말을 한다. 6.6초짜리 가상 음성 연출이며 31번의 통화 장면은 그대로 유지한다. P는 재생·일시정지, A는 음소거다. 제작 기록은 `public/abuse/audio/threshold-voices.json`에 있다.
27번은 장표 진입 뒤 약 1.35초 쉬고 23번에서 들은 실제 강의의 한 문장을 자동으로 다시 들려준 뒤, "저는 이 문장을 녹음한 적이 없습니다"로 붉게 바꾸고,
...
31번 수신 시 짧은 진동음(WebAudio)이 두 번 나고, `→` 또는 휴대전화의 응답 버튼을 누르면 통화 단계로 넘어가 가상 합성 음성이 한 번 재생된다. 32번 음성 메시지도 장표 진입 시 한 번 재생된다. A는 음소거, P는 재생/정지다.
두 음성은 Qwen3-TTS와 발표자 본인의 LoRA 어댑터(0.60)로 만들었으며 실제 다른 사람의 목소리를 흉내 내지 않는다. 제작 기록은 `public/abuse/audio/manifest.json`에 있다.
33번은 모자이크한 사진 한 장이 두 배씩 늘어 화면을 채우고, 원본을 지워도 사본이 남는다.
34번은 같은 휴대전화에서 끊고 저장된 번호로 다시 거는 확인 방법(FTC 권고)으로 끝난다. 모든 통화·메시지·사진은 가상 상황이다.
