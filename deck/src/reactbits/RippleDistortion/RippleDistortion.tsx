// Adapted from React Bits (David Haz): stage coordinates, idle rendering, local fallback.
...
const RippleDistortion = ({
  src = '/performance/future-surface.svg',
  brushSize = 150,
  strength = 0.2,
  swirl = 1,
  rings = 4,
  ...
  configRef.current = { brushSize, spread, fade, spacing, clickStrength, trigger, enabled, pulseKey };
  ...
  useEffect(() => {
    ...
    if (!(probe.getContext('webgl2', { preserveDrawingBuffer: true, alpha: false, antialias: false }) || probe.getContext('webgl', { preserveDrawingBuffer: true, alpha: false, antialias: false }))) {
    ...
    let disposed = false;
    let raf = 0, previousTime = 0, frames = 0;
    const wake = () => { if (!raf && !disposed) { previousTime = 0; raf = requestAnimationFrame(loop); } };
    const lost = (event: Event) => { event.preventDefault(); cancelAnimationFrame(raf); raf = 0; disposed = true; canvas.style.opacity = '0'; mount.dataset.fallback = 'true'; };
    ...
    const localPoint = (clientX: number, clientY: number): [number, number] | null => {
      ...
      return [(clientX - rect.left) * width / rect.width, (rect.height - (clientY - rect.top)) * height / rect.height];
    };
    ...
    function loop(now: number) {
      ...
      const active = waves.some(wave => wave.opacity > 0);
      canvas.dataset.active = String(active);
      if (active && !raf) raf = requestAnimationFrame(loop);
    }
    const pulse = () => { const cfg = configRef.current; if (cfg.enabled && !reduceMotion) setNewWave(width / 2, height / 2, cfg.clickStrength); };
    mount.addEventListener('ripple-pulse', pulse);
    wake();
    ...
    return () => {
      ...
      mount.removeEventListener('ripple-pulse', pulse);
  ...
  useEffect(() => {
    if (pulseKey) mountRef.current?.dispatchEvent(new Event('ripple-pulse'));
  }, [pulseKey]);
  ...
  return <div ref={mountRef} className={`ripple-distortion ${className}`.trim()} style={{ background: `url(${src}) center / cover`, ...style }} />;
};
