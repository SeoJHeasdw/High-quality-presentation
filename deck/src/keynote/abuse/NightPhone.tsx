...
/*
 * 36·37·39번의 무대. Blender로 렌더한 한밤중 협탁(public/abuse/night-phone.jpg) 위에서
...
const FAKE_LINES = ["엄마… 나야.", "나 사고 났어.", "폰이 깨져서 친구 폰으로 전화했어.", "지금 좀 급해. 엄마, 나 좀 도와줘.", "아빠한테는 말하지 마."];
...
export function PhoneScreen({ mode, onAnswer }: { mode: PhoneMode; onAnswer?: () => void }) {
  if (mode === "incoming") return <div className="ph ph--incoming">
    ...
    <div className="ph-actions"><span className="ph-btn ph-btn--decline"><i/>거절</span><button type="button" className="ph-btn ph-btn--accept" onClick={onAnswer} aria-label="전화를 받아 통화 장면으로 이동"><i/>응답</button></div>
  </div>;
  ...
  if (mode === "chat" || mode === "chat-more") return <div className="ph ph--chat" data-more={mode === "chat-more" || undefined}>
    ...
    <div className="ph-thread">
      <div className="ph-bubble ph-bubble--voice" style={{ "--i": 0 } as CSSProperties}><i className="ph-play"/><FakeWave bars={22}/><span>0:05</span></div>
  ...
  if (mode === "hangup") return <div className="ph ph--hangup">
    ...
    <div className="ph-suggest"><span>확인하기</span><div className="ph-contact"><i/><div><strong>우리 아들</strong><span>저장된 번호로 다시 걸기</span></div><b/></div></div>
  </div>;
  return <div className="ph ph--callback">
    <StatusBar time="2:49"/>
    <div className="ph-caller"><span className="ph-caller__kind">저장된 번호</span><strong>우리 아들</strong><span className="ph-timer ph-timer--ok">통화 중</span></div>
    <FakeWave red={false}/>
...
/** 수신 장면에 들어가면 낮은 진동음이 두 번 난다. A로 끌 수 있다. */
function useBuzz(active: boolean, sound: boolean) {
  useEffect(() => {
    ...
    for (let k = 0; k < 2; k++) { // 입장 때 두 번만 울리고 발표자가 말할 공간을 남긴다
...
export default function NightPhone({ mode, tone = "cold", children }: { mode: PhoneMode; tone?: "cold" | "red" | "warm"; children?: ReactNode }) {
  ...
  // 장식 층만 숨긴다. 37번의 살펴보기 버튼(children)은 보조 기술에서도 누를 수 있어야 한다.
  return <div ref={root} className="np" data-mode={mode} data-tone={tone} data-motion={motion || undefined}
    ...
    <div className="np-cam"><div className="np-stage">
      ...
      <div className="np-shake"><div className="np-screen" style={{ transform: MATRIX, width: UI_W, height: UI_H }}><PhoneScreen mode={mode} onAnswer={() => window.dispatchEvent(new CustomEvent("deck:step", { detail: 1 }))}/><i className="np-sheen"/></div></div>
      {children}
    </div></div>
    <div className="np-grain" aria-hidden="true"/>
    <div className="np-vignette" aria-hidden="true"/>
    <span className="np-sound" aria-hidden="true" data-on={sound || undefined}>{mode === "incoming" ? sound ? "진동음 켜짐 · A" : "진동음 꺼짐 · A" : ""}</span>
  </div>;
}
